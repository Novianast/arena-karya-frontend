...
import { Home, CreditCard, Package, ChevronRight, Calendar, User, Briefcase, Gavel } from "lucide-react";
...
export default function AdminLayout({
      ...
      if (data && !error) {
        ...
        if (data.profile_image) {
          const { data: publicUrlData } = supabase.storage
            .from("profiles")
            .getPublicUrl(`admins/${data.profile_image}`);
  ...
  const menuItems = [
    ...
    { name: "Pembayaran", path: "/admin/payment", icon: CreditCard },
    { name: "Paket", path: "/admin/package", icon: Package },
    { name: "Event", path: "/admin/event", icon: Calendar },
    { name: "Penyelenggara", path: "/admin/organizer", icon: Briefcase },
    { name: "Juri", path: "/admin/judge", icon: Gavel },
    { name: "Peserta", path: "/admin/participant", icon: User },
  ];
    ...
    <div className="flex h-screen w-full overflow-hidden">
      ...
      <div className="flex-1 flex flex-col h-full overflow-hidden">
        ...
        <header className="h-[88px] w-full bg-white border-b border-padded-white px-8 flex items-center justify-between shrink-0">
          ...
          {/* Bagian Kanan Header (Foto Profil) */}
